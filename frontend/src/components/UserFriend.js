...
export const UserFriend = (props) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('requestAccepted', (data) => {
      ...
      if (data.requesterId === currentUserId) {
        ...
        navigate(`/matchwait/?requesterId=${data.requesterId}&receiverId=${data.receiverId}`)
  ...
  const handleAccept = () => {
    ...
    navigate(`/matchwait/?requesterId=${matchRequestData.requesterId}&receiverId=${matchRequestData.userId}`)
